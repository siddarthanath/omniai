...
Context: To test various dimensional datasets.
"""
...
from src.optimisers.analytical.configs import GradientDescentConfig
from src.optimisers.analytical.base import GradientOptimiser
from src.trainers.analytical.gradient_trainer import GradientTrainer
...
def test_lr_2d(test_case, X_fn, add_noise):
    ...
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(x=X_standardized.flatten(), y=y, mode="markers", name="Data Points")
    )
    X_sorted = np.sort(X_standardized, axis=0)
    y_pred = model.predict(X_sorted)
    fig.add_trace(
        go.Scatter(x=X_sorted.flatten(), y=y_pred, mode="lines", name="Fitted Line")
    )
    fig.update_layout(
...
@pytest.mark.parametrize(
    ...
    [
        (
            "gaussian_no_noise",
            lambda n: np.column_stack([np.linspace(-3, 3, n), np.linspace(-2, 2, n)]),
            False,
        ),
        (
            "gaussian_with_noise",
...
def test_lr_3d(test_case, X_fn, add_noise):
    ...
    fig.add_trace(go.Surface(x=x1_mesh, y=x2_mesh, z=y_mesh, opacity=0.5))
    fig.update_layout(
